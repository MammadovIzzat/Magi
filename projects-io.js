...
function serializeTarget(a) {
  ...
  return {
    ...
    findings: db.prepare(`SELECT id,uid,title,kind,severity,body,refs,fix_status,in_report,author,created_at FROM findings WHERE asset_id=? ORDER BY id`).all(a.id)
      .map(f => ({
        uid: f.uid, title: f.title, kind: f.kind, severity: f.severity, body: f.body, refs: f.refs, fix_status: f.fix_status, in_report: f.in_report, author: f.author, created_at: f.created_at,
...
export function importProject(bundle, nameOverride) {
  ...
  try {
    ...
    const insFinding = db.prepare(`INSERT INTO findings (asset_id,title,kind,severity,body,fix_status,in_report,author,created_at) VALUES (?,?,?,?,?,?,?,?,?)`);
      ...
      for (const tgt of (folder.targets || [])) {
        ...
        for (const f of (tgt.findings || [])) {
          ...
          const fnd = insFinding.run(aid, f.title, f.kind || 'note', f.severity ?? null, f.body ?? null,
            fix, f.in_report ? 1 : 0, f.author ?? null, f.created_at || new Date().toISOString()).lastInsertRowid;
          nFindings++;
