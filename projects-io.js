...
function serializeTarget(a) {
  ...
  return {
    ...
    findings: db.prepare(`SELECT id,uid,title,kind,severity,body,refs,fix_status,in_report,author,cvss,flagged_to,created_at FROM findings WHERE asset_id=? ORDER BY id`).all(a.id)
      .map(f => ({
        uid: f.uid, title: f.title, kind: f.kind, severity: f.severity, body: f.body, refs: f.refs, fix_status: f.fix_status, in_report: f.in_report, author: f.author, cvss: f.cvss, flagged_to: f.flagged_to, created_at: f.created_at,
...
export function importProject(bundle, nameOverride) {
  ...
  try {
    ...
    const insTarget = db.prepare(`INSERT INTO assets (project_id,folder_id,type,label,metadata,assignee,notebook,created_at) VALUES (?,?,?,?,?,?,?,?)`);
    ...
    for (const folder of foldersFromBundle(bundle)) {
      ...
      for (const tgt of (folder.targets || [])) {
        ...
        const aid = insTarget.run(pid, fid, tgt.type, tgt.label, JSON.stringify(tgt.metadata || {}),
          tgt.assignee || null, tgt.notebook || null, tgt.created_at || new Date().toISOString()).lastInsertRowid;
        nTargets++;
        ...
        for (const f of (tgt.findings || [])) {
          ...
          const fnd = insFinding.run(aid, f.title, f.kind || 'note', f.severity ?? null, f.body ?? null,
            fix, f.in_report ? 1 : 0, f.author ?? null, f.cvss ?? null, f.flagged_to ?? null, f.created_at || new Date().toISOString()).lastInsertRowid;
          nFindings++;
